      ...
      <div className="grid md:grid-cols-2 gap-8">
        {PROJECTS.map((project, idx) => (
          <motion.div 
            whileHover={{ y: -5 }}
            key={idx} 
            className="bg-dark-900 p-8 rounded-lg border border-white/5 hover:border-gold-500/30 transition-all"
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-4 text-gray-400">
                <a href={project.links.github} target="_blank" className="hover:text-gold-500"><Github size={20} /></a>
                <a href={project.links.demo} className="hover:text-gold-500"><ExternalLink size={20} /></a>
              </div>
            </div>
            <h4 className="text-xl font-bold text-white mb-2">{project.title}</h4>
            <p className="text-gray-400 text-sm mb-6 leading-relaxed h-20">{project.description}</p>
